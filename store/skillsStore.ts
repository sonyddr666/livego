...
const DEFAULT_SKILLS: Skill[] = [
  {
    ...
    functionDescription: 'Exibe uma imagem real na tela do usuario. REGRAS: 1) Se voce VE uma URL de imagem na tela do usuario (via screen share), copie essa URL EXATA no campo query. 2) Se voce encontrou URLs de imagem nos resultados do Google Search, use essas URLs diretamente no query. 3) Qualquer URL que comeca com http sera exibida diretamente. 4) Se nao tiver URL, passe um termo CURTO em INGLES (max 3 palavras). NUNCA use frases longas em portugues como query.',
    parameters: {
      query: {
        type: 'string',
